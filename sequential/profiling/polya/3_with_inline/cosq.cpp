...
// #define BSC_ERROR_PROBABILITY 0
...
//     x >>= 1;
//   }
//   return pow(BSC_ERROR_PROBABILITY, count) * pow(1-BSC_ERROR_PROBABILITY, num_bits - count);
// }
...
int main(int argc, char** argv) {
    ...
    int normal_bitrate = atoi(argv[3]);
    int laplacian_bitrate = atoi(argv[4]);
    printf("Training %d bit laplacian, %d bit normal quantizers for polya channel delta %f, epsilon %f\n", laplacian_bitrate, normal_bitrate, polya_delta, polya_epsilon);
    ...
    cosq(normal, normal_bitrate, quantizers, polya_urn_error);
    cosq(laplace, laplacian_bitrate, quantizers +  MAX_CODEBOOK_SIZE, polya_urn_error);
    free(normal);
    free(laplace);
    return 0;
}